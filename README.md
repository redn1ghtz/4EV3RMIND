...
* **Полное управление нейросетью** - все решения принимает выбранная модель OpenRouter
...
```bash
git clone https://github.com/redn1ghtz/4EV3RMIND.git -b OpenRouter-API
cd 4EV3RMIND
```
...
```bash
vi 4EV3RMIND/openrouter_config.py
```
OPENROUTER_API_KEY = "your-openrouter-api-key" вместо your-openrouter-api-key вставьте Ваш API-ключ, полученный на [сайте](https://openrouter.ai)
#### Основные настройки
```python
OPENROUTER_API_KEY = "your_openrouter_api_key"     # Ключ OpenRouter API
...
#### Лимиты
```python
DAILY_REQUEST_LIMIT = 1000         # Запросов в день
