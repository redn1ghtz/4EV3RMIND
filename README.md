...
* **Полное управление нейросетью** - все решения принимает модель от Algion.dev (по умолчанию gpt-5.1)
...
```bash
git clone https://github.com/redn1ghtz/4EV3RMIND.git -b Algion-API
cd 4EV3RMIND
```
...
cd 4EV3RMIND
```
8) Запустите установщик (он установит необходимые зависимости и настроит русский язык в системе). После завершения установки система робота будет перезагружена автоматически. Установка занимает 20-30 минут при хорошем интернет соединении и хорошей скорости чтения/записи micro-SD карты:
```bash
sudo ./install.sh
```
9) С помощью vi отредактируйте конфигурационный файл:
```bash
vi 4EV3RMIND/algion_config.py
```
ALGION_API_KEY = "your-algion-api-key" вместо your-algion-api-key вставьте Ваш API-ключ, полученный у Algion, инструкция: [GitHub проекта](https://github.com/Mr-Abood/GPTFree)
#### Основные настройки
```python
ALGION_API_KEY = "your-algion-api-key"     # Ключ Algion API
...
#### Лимиты
```python
DAILY_REQUEST_LIMIT = 14400         # Запросов в день (отключен по умолчанию)
